...
export const appRouter = router({
  ...
  profile: router({
    ...
    get: publicProcedure
      ...
      .query(async ({ input }) => {
        const profile = await supabaseDb.getFounderProfileByUserId(
          input.supabaseId
        );
        return profile;
      }),
      ...
      .query(async ({ input }) => {
        ...
        if (input.currentUserSupabaseId) {
          result.data = result.data.filter(
            p => p.user_id !== input.currentUserSupabaseId
          );
    ...
    delete: protectedProcedure
      ...
      .mutation(async ({ input, ctx }) => {
        ...
        await supabaseDb.deleteFounderProfile(
          ctx.user.accessToken,
          ctx.user.id
        );
        return { success: true };
