...
import { Search, ExternalLink, Code, Lightbulb, Clock, Briefcase, Linkedin } from 'lucide-react';
  ...
  const filteredProfiles = useMemo(() => {
    ...
    return profiles.filter((profile) => {
      ...
      const matchesSkills =
        ...
        (profile.skill_areas && skillFilters.some(skill => profile.skill_areas?.includes(skill)));
  ...
  return (
    <>
      ...
      <div className="min-h-screen flex flex-col">
        {/* Header - Mobile first */}
        <header className="px-4 py-4 border-b sm:px-6 sm:py-6 bg-white/80 backdrop-blur-sm sticky top-0 z-10">
          <div className="container flex items-center justify-between gap-2">
            <div className="flex items-center gap-2 min-w-0">
              <LogoIcon className="h-6 w-6 flex-shrink-0 sm:h-8 sm:w-8" />
              <span className="font-semibold lowercase text-sm truncate sm:text-base">{APP_TITLE.toLowerCase()}</span>
            </div>
        ...
        <main className="flex-1 px-4 py-6 sm:px-6 sm:py-8">
          <div className="container max-w-7xl">
            <div className="mb-6 sm:mb-8">
              <h1 className="text-2xl font-bold lowercase mb-2 sm:text-3xl">find your co-founder</h1>
              <p className="text-sm text-muted-foreground sm:text-base">
                {filteredProfiles.length} founder{filteredProfiles.length !== 1 ? 's' : ''} in hamburg
              </p>
            </div>

            {/* Filters - Mobile first, stack vertically on mobile */}
            <div className="mb-6 space-y-3 sm:space-y-4">
              {/* Search and Filter Dropdowns - Stack on mobile, row on desktop */}
              <div className="flex flex-col gap-3 lg:flex-row lg:items-center">
                ...
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-10 lowercase text-sm sm:text-base"
                  />
                </div>

                {/* Filter Dropdowns - Side by side on all screens */}
                <div className="flex gap-3 lg:flex-shrink-0">
                  <Select value={technicalFilter} onValueChange={setTechnicalFilter}>
                    <SelectTrigger className="lowercase text-sm sm:text-base w-full lg:w-[180px]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="all">all founders</SelectItem>
                      <SelectItem value="technical">technical only</SelectItem>
                      <SelectItem value="non_technical">non-technical only</SelectItem>
                    </SelectContent>
                  </Select>
                  ...
                  <Select value={ideaFilter} onValueChange={setIdeaFilter}>
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="looking_for_idea">looking for idea</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              <div className="border rounded-lg p-4">
                <h3 className="text-sm font-medium lowercase mb-3">filter by skills</h3>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
                  {FOUNDER_SKILLS.map((skill) => (
                    <div key={skill.value} className="flex items-center space-x-2">
                      <Checkbox
                        id={skill.value}
                        checked={skillFilters.includes(skill.value)}
                        onCheckedChange={(checked) => {
                          setSkillFilters(
                            checked
                              ? [...skillFilters, skill.value]
                              : skillFilters.filter((s) => s !== skill.value)
                          );
                        }}
                      />
                      <Label
            ...
            {isLoading && (
              <div className="text-center py-12">
                <div className="text-muted-foreground lowercase">loading founders...</div>
              </div>
            )}
            ...
            {!isLoading && filteredProfiles.length === 0 && (
              <Card>
                <CardContent className="py-12 text-center">
                  <p className="text-muted-foreground lowercase mb-4">no founders found</p>
                  <Button onClick={() => { setSearchQuery(''); setTechnicalFilter('all'); setIdeaFilter('all'); setSkillFilters([]); }}>
                    clear filters
                  </Button>
                </CardContent>
              </Card>
                  ...
                  <CardHeader className="pb-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <CardTitle className="text-base lowercase mb-2">{profile.name}, {profile.age}</CardTitle>
                  ...
                  <CardContent className="space-y-3 text-sm">
                    ...
                    {profile.idea && (
                      <div>
                        ...
                        <p className="text-sm line-clamp-2">{profile.idea}</p>
                      </div>
                    )}
            ...
            <div className="hidden lg:block">
              <Card>
                <div className="overflow-x-auto thin-scrollbar">
                  ...
                  <TableHeader>
                    <TableRow>
                      <TableHead className="lowercase whitespace-nowrap min-w-[180px]">founder</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[120px]">type</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[140px]">status</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[160px]">commitment</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[200px]">skills</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[200px]">looking for</TableHead>
                      <TableHead className="lowercase whitespace-nowrap min-w-[100px]">contact</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {filteredProfiles.map((profile) => (
                      <TableRow key={profile.id}>
                        ...
                        <TableCell className="min-w-[180px]">
                          <div className="flex items-center gap-3">
                            ...
                            <div className="min-w-0">
                              <div className="font-medium lowercase truncate">{profile.name}</div>
                        ...
                        <TableCell className="min-w-[200px]">
                          {profile.skill_areas && profile.skill_areas.length > 0 ? (
                            <div className="flex flex-wrap gap-1">
                              {profile.skill_areas.map((skill) => (
                                <Badge key={skill} variant="outline" className="text-xs lowercase whitespace-nowrap">
                                  {skill.replace('_', ' & ')}
                                </Badge>
                              ))}
                            </div>
                        ...
                        <TableCell className="min-w-[200px] max-w-[300px]">
                          <div className="w-full max-w-[280px]">
                            <p className="text-sm break-words whitespace-normal">{profile.looking_for || '-'}</p>
                          </div>
                        </TableCell>
